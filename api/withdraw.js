...
const GAS_LIMIT = BigInt('30000000000000'); // Достаточно газа для двух вызовов
// Минимальный депозит для регистрации аккаунта (0.00125 NEAR)
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Если null, аккаунт не зарегистрирован -> добавляем storage_deposit
        if (isRegistered === null) {
            ...
            actions.push(
                transactions.functionCall(
                    ...
                    GAS_LIMIT,
                    MIN_STORAGE_DEPOSIT // Депозит 0.00125 NEAR
                )
            );
        }
        ...
        actions.push(
            transactions.functionCall(
                ...
                GAS_LIMIT,
                ONE_YOCTO // 1 yoctoNEAR
            )
        );

        // 4. Выполнение пакетной транзакции
        const result = await account.signAndSendTransaction({
            receiverId: TOKEN_CONTRACT_ID, // Контракт, который вызывает оба действия
            actions: actions,
        });
